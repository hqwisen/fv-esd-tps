...
WATCH_TIME_TYPE INITIAL_WATCH_TIME; /* initialized in _main */
...
#define  _main1_col  (_XW-1)
#define  _main2_col  (_XW+2)
#define  _main3_col  (_XW+5)
#define  mini1_col  (_XW+2)
#define  mini2_col  (_XW+5)
#define llabels_col (_XW-20)
#define rlabels_col (_XW+9)
short X_ENHANCE[] = {_main3_col, _main1_col+1, _main2_col,_main2_col+1,
...
void New_Watch_O_display(void* dummy, DISPLAY_TYPE disp)
{
	int hpos;
/* Main display */
	move(lower_line,_main1_col);
	addstr(disp._main.hours);
	move(lower_line,_main2_col);
	addstr(disp._main.minutes);
	move(lower_line,_main3_col);
	addstr(disp._main.seconds);
/* 24H/AM-PM, Stopwatch or Alarm status */
	move(medium_line,alpha_col);
	addstr(disp._main.timemode);
/* Mini display */
...
void MAKE_DISPLAY(
   ...
   _assign_MAIN_DISPLAY_TYPE(&(res->_main), _main);
...
void WATCH_TIME_TO_MAIN_DISPLAY(
   ...
   static MAIN_DISPLAY_TYPE _maindt;
   static int hours;
/* in AM-PM mode, the time is converted */
   ...
   sprintf(_maindt.hours,"%2d",hours);
   sprintf(_maindt.minutes,"%02d",wt.minutes);
   sprintf(_maindt.seconds,"%02d",wt.seconds);
/* The 24H/AM-PM mode */
   if (wt.mode12h) {
      if (wt.hours>11) strcpy(_maindt.timemode,"PM ");
      else strcpy(_maindt.timemode,"AM ");
   } else strcpy(_maindt.timemode,"24H");
   * res = _maindt;
}
...
seconds, 1/100sec., othrwise it is displayed in hours, minutes, seconds */
   if (st.hours) {
      sprintf(_maindt.hours,"%2d",st.hours);
      sprintf(_maindt.minutes,"%02d",st.minutes);
      sprintf(_maindt.seconds,"%02d",st.seconds);
      strcpy(_maindt.timemode,"   ");
   } else {
      sprintf(_maindt.hours,"%2d",st.minutes);
      sprintf(_maindt.minutes,"%02d",st.seconds);
      sprintf(_maindt.seconds,"%02d",st.hsec);
      strcpy(_maindt.timemode,"   ");
   };
   *res = _maindt;
}
...
/* in AM-PM mode, the time is converted */
   ...
   sprintf(_maindt.hours,"%2d",hours);
   sprintf(_maindt.minutes,"%02d",at.minutes);
   sprintf(_maindt.seconds,"  ");
/* The 24H/AM-PM mode */
   if (at.mode12h) {
     if (at.hours>11)
        strcpy(_maindt.timemode,"PM ");
        else strcpy(_maindt.timemode,"   ");
   } else strcpy(_maindt.timemode,"24H");
   *res = _maindt;
}
